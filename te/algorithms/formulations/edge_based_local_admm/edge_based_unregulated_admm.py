...
class UnregulatedADMMLP(TrafficEngineeringLP):
    def __init__(self, graph: nx.DiGraph, traffic: TrafficMatrixBase, solver_params: UnregulatedADMMSolverParams) -> None:
        ...
        self._check_result: Optional[TrafficEngineeringLPEvaluationParams] = None
    ...
    def _update_P_bar(self):
        ...
        The update rule for `P_bar` is:
            ...
            P_bar <-- (NULL_M^T F + alpha * (eta / rho) (u + alpha * Y_bar)) / (K + alpha^2 * (eta / rho))
        """
    ...
    def _update_r_e(self):
        ...
        The update rule for r_e is:
            r_e <-- r_e + alpha(X_oe - sum_k [X_ke])/2
        """
    ...
    def check(self, eval_params: TrafficEngineeringLPEvaluationParams):
        ...
        ZO_E = self._Zo_e
        outer_admm_consensus_test(XO_E, ZO_E, eval_params)
