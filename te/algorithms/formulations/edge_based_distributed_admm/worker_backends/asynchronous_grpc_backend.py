...
class AsynchronousgRPCBackend(WorkerNodeCommunicationBackendBase):
    ...
    def start(self):
        ...
        self._event_loop.run_until_complete(self._server.start())
        self.is_worker_node_ready = True

    def stop(self):
        self.is_worker_node_ready = False
        if self._server is not None:
            self._event_loop.run_until_complete(self._server.stop(1))
    
    def wait(self):
        if self._server is not None:
            self._event_loop.run_until_complete(self._server.wait_for_termination())


class NetworkWorkerNodeListener(DistributedADMMSolverServicer):
    def __init__(self, backend: AsynchronousgRPCBackend):
        super().__init__()
        self._backend = backend
        self._id = backend.worker_id
    
    async def SetInitialFeasibleSolution(self, request_iterator: Iterator[distributed_lp_messages.Chunk], context):
        self._backend.set_initial_feasible_solution(rebuild_chunked_array(request_iterator))
        return Empty()

    async def SetNullSpaceBasis(self, request_iterator: Iterator[distributed_lp_messages.Chunk], context):
    ...
    async def RequestChunk(self, request, context):
        return chunk_big_array(self._backend.report_chunk(), GRPC_ARRAY_STREAM_MAX_LEN)
    
    async def RequestAggregate(self, request, context):
    ...
    async def SetSolverParameters(self, request: distributed_lp_messages.SolverParameters, context):
