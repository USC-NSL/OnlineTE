...
#     BIG_N = n @ n.T + 1e-3 * np.eye(num_edges)
#     b = -np.linalg.inv(BIG_N) @ (x_0 + n @ c)
...
def active_set_dual_test(n: np.ndarray, c: np.ndarray, x_0: np.ndarray) -> Tuple[np.ndarray, float]:
    ...
    nnt = n @ n.T + 1e-3 * np.eye(n.shape[0])
...
def admm_test_primal(n: np.ndarray, c: np.ndarray, x_0: np.ndarray, num_iters: int, gamma: float) -> Tuple[np.ndarray, np.ndarray, float]:
    """
    ADMM solution.
    """
    ...
    Y_k = np.zeros((null_dim,))
    S_k = np.zeros((num_edges,))
    T_k = np.zeros((num_edges,))
    i = 0
    start = time.time()
    for i in range(num_iters):
        Y_k = (c - gamma * n.T @ (x_0 + T_k - S_k)) / (1 + gamma)
        S_k = np.clip(x_0 + n @ Y_k + T_k, a_min=0, a_max=None)
        T_k = T_k + (x_0 + n @ Y_k - S_k)
    return Y_k, S_k, time.time() - start
...
if __name__ == '__main__':
    ...
    assert np.linalg.det(n @ n.T + 1e-3 * np.eye(number_of_edges)) > 0
    _, null_dim = np.shape(n)
    ...
    # The active set method over the dual
    ...
    # ADMM over the primal
    admm_primal, admm_slack, admm_primal_runtime = admm_test_primal(n, c, x_0, 100, 1)
    ...
    print(f"ADMM primal: {str(round(admm_primal_runtime, 3))}")
    ...
    print(f"Primal ADMM gap: < DON'T KNOW! >")
    ...
    print(f"Gurobi/ADMM gap: {np.abs(get_primal_objective_from_feasible_solution(c, admm_primal) - optimal_primal_obj)}")
    ...
    print(f"Gurobi/ADMM-primal solution gap: {np.linalg.norm(admm_primal - primal) / np.sqrt(null_dim)}")
    ...
    print(f"ADMM infeasibility: {np.linalg.norm(np.clip(admm_slack, a_min=None, a_max=0.0))}")
