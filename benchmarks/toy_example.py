...
from utils.draw_assignments import set_graph_weights_to_zero, draw_directed_weighted_graph, get_assignments_for_particular_commodity
...
def draw_final_assignment(problem: TrafficEngineeringProblemDescription, trace: TrafficEngineeringLPObjectiveTrace):
    ...
    capacity = graph.get_edge_data(0, 1)['capacity'] * final_util
    commodity_dict = {
        (item.source, item.destination):
            (index, item.demand) for index, item in enumerate(traffic_to_commodity(problem.TM))
    }
    plt.figure()
    draw_directed_weighted_graph(get_assignments_for_particular_commodity(
        source=0, destination=3, graph=graph, capacity=capacity, 
        assignments=final_assignment, commodity_dict=commodity_dict
    ))
    plt.show()
...
if __name__ == '__main__':
    ...
    trace = spawn_distributed_synchronous_solver(problem, solver, True)
    draw_final_assignment(problem, trace)
